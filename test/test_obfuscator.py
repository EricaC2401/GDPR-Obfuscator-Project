...
class TestObfuscateFieldsInDf:
    ...
    @pytest.mark.it(
        "Test if obfuscated fields in the list" + " with method equals mask"
    )
    ...
    @pytest.mark.it(
        "Test if obfuscated fields in the list" + " with method equals hash"
    )
...
class TestConvertStrFileToCSV:
    ...
    def test_process_parquet_called(
        ...
        test_content, test_fields = test_parquet_data
        output = convert_str_file_content_to_obfuscated_csv(
            test_content, test_fields, "parquet"
        )
        assert isinstance(output, io.BytesIO)
...
class TestObfuscateFile:
    ...
    def test_inner_functions_are_called(
        self, mock_convert_csv_output, mock_convert_str_csv, test_csv_data
    ):
        test_content, test_fields = test_csv_data
